...
if (isset($_GET['rfid'])) {
    ...
    $sql = "SELECT status, profile FROM registrants WHERE rfid = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt->bind_result($status, $profile);
        $stmt->fetch();
        ...
        if ($updateStmt->execute()) {
            ...
            $insertSql = "INSERT INTO entries (registrants_rfid, status, profile) VALUES (?, ?, ?)";
            ...
            $insertStmt->bind_param("sis", $rfid, $newStatus, $profile);
        ...
        $notFoundSql = "INSERT INTO entries (registrants_rfid) VALUES( ? )";
